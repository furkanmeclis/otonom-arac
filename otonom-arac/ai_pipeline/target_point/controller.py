"""Simple controller for target-point inference."""
...
def target_point_to_controls(
    ...
    """Convert a forward target point into steering and throttle.

    When *dynamic_throttle* is enabled the throttle varies continuously
    between *base_throttle* (on straights) and *min_throttle* (on sharp
    curves) based on the predicted heading error.
    """
...
class TargetPointController:
    """DonkeyCar part that turns predicted target points into drive commands.

    Adds temporal intelligence on top of the instantaneous geometric
    controller:

    1. **Anticipatory throttle** — keeps a sliding window of recent
       heading-error magnitudes.  When the trend is *rising* (curve
       approaching) the effective curvature used for throttle is boosted
       so the car brakes early.  The steering itself stays reactive so
       tracking precision is preserved.

    2. **Steering-rate limiting** — caps the maximum frame-to-frame
       change in steering output, removing sudden jerks without the
       phase-lag of a low-pass filter.
    """
