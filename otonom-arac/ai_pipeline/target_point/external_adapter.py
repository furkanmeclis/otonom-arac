"""Convert external Donkey Car tub datasets to target-point training format.

Reads tub catalogs (user/angle, user/throttle) and synthesises pseudo
target-point labels using the inverse of the geometric controller:

    heading_error = atanh(angle) / steer_gain
    target_x      = lookahead_y * tan(heading_error)
    target_y      = lookahead_y                        (constant)
"""
...
def angle_to_target_point(
    ...
    """Invert the geometric controller to get (target_x, target_y).

    The forward controller does:
        heading_error = atan2(target_x, target_y)
        steering      = tanh(steer_gain * heading_error)

    Inverse:
        heading_error = atanh(steering) / steer_gain
        target_x      = target_y * tan(heading_error)

    heading_error is capped at ±60° to prevent tan() blowup when
    steering values are close to ±1 (atanh diverges near ±1).
    """
...
def convert_tub_to_samples(
    ...
    *,
    ...
    """Convert a single tub directory into TargetPointSample list + quality stats."""
...
def scan_external_datasets(
    ...
    *,
    ...
    """Scan all tub directories under root_dir and return train/val splits."""
...
def write_external_manifest(
    ...
    *,
    ...
    """Scan external datasets and write a JSONL manifest file."""
