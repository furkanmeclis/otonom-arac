// ============================================================
// BIRLESİK.SCAD — Montaj Önizlemesi  v1.0
// SADECE görsel kontrol içindir. Baskı için:
//   → kapak.scad         (üst kapak)
//   → kamera_braketi.scad (kamera braketi)
//
// F5 = hızlı önizleme (noktalı çizgiler normal)
// F6 = tam katı render (gerçek şekil — bunu kullanın)
// ============================================================
...
module kapak() {
    ...
    difference() {
        ...
        for (dx=[M4MX,L-M4MX], dy=[M4MY,W-M4MY])
            translate([dx,dy,H-T-0.1]) cylinder(d=M4D,h=T+0.2);
        // Servis kapağı açıklığı
...
module kamera_yuvasi() {
    difference() {
        ...
        for (lx=[CAM_LN*0.3,CAM_LN*0.7])
            translate([lx,0,0]) rotate([90,0,0])
    ...
    difference() {
        union() {
            ...
            translate([BASE_D,BASE_W/2,BASE_T+ARM_H])
                rotate([0,CAM_ANG,0]) kamera_yuvasi();
        }
        for (dy=[BASE_W/2-20,BASE_W/2+20])
            translate([BASE_D/2,dy,-0.1]) cylinder(d=M3D,h=BASE_T+0.2);
        translate([BASE_D/2-CB/2,BASE_W/2-CB/2,BASE_T-0.1])
            cube([CB,CB,ARM_H+2]);
        // Yatay bağlantı slotu
...
// ============================================================
// BİRLEŞİK GÖRÜNÜM — Kapak + Braket
// Kapak z=0'da. Braket kapak üstünde (z=H), ön kenara hizalı.
// ============================================================
color("DimGray",0.85) kapak();
color("SlateGray",0.9)
