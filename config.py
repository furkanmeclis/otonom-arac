...
DONKEY_GYM = False   # GERÇEK ARAÇ: sim kapalı. (Simülasyon için simulationconfig.py kullanılır.)
...
# =====================================================================
# TARGET POINT — GERÇEK ARAÇ SÜRÜŞ AYARLARI
# ---------------------------------------------------------------------
# Bu blok gerçek araçta hedef-nokta modelini sürmek için ZORUNLU.
# (simulationconfig.py'de bunlar var ama orası SİM içindir; config.py = GERÇEK.)
#
# ⚠️ EN KRİTİK: IMAGE_W/H = 128 olmalı — modellerin 128x128 ile eğitildi.
#    (simulationconfig.py'de 224 yazıyor; gerçek araçta 128 şart, yoksa model
#     yanlış boyutta görüntü alır ve saçmalar.)
#
# NOT: Yukarıda CAMERA_TYPE = "PICAM". Jetson CSI kamera için "CSIC",
#      USB kamera için "WEBCAM" yap. PWM değerlerini `donkey calibrate` ile ölç.
# =====================================================================

TARGET_POINT_ENABLED = True
TARGET_POINT_MODEL_ARCH = 'efficient'
TARGET_POINT_IMAGE_W = 128              # ⚠️ model 128 ile eğitildi (224 DEĞİL)
TARGET_POINT_IMAGE_H = 128
TARGET_POINT_LOOKAHEAD_METERS = 1.0

# Görüntü kırpma — eğitimle AYNI olmalı (model bu kırpmayla öğrendi)
TARGET_POINT_CROP_TOP = 10
TARGET_POINT_CROP_BOTTOM = 0
TARGET_POINT_CROP_LEFT = 0
TARGET_POINT_CROP_RIGHT = 0

# --- Kontrolcü (hedef nokta -> direksiyon/gaz) ---
TARGET_POINT_STEER_GAIN = 1.35          # direksiyon şiddeti (virajı kaçırırsa artır, zikzakta azalt)
TARGET_POINT_STEER_SIGN = 1.0           # araç TERS kırarsa -1.0 yap
TARGET_POINT_TARGET_X_BIAS_M = 0.08     # hep bir yana çekerse bununla ortala
...
TARGET_POINT_STEER_RATE_LIMIT = 0.15    # kareler arası ani direksiyon sıçramasını sınırla

# --- Gaz / hız (küçük pist için düşük tutuldu) ---
TARGET_POINT_THROTTLE = 0.20
TARGET_POINT_DYNAMIC_THROTTLE = True    # virajda otomatik yavaşla
TARGET_POINT_BASE_THROTTLE = 0.18       # düz yol hızı (ilk testte düşük başla)
TARGET_POINT_MIN_THROTTLE = 0.07        # keskin virajda hız
...
TARGET_POINT_ANTICIPATION_FRAMES = 10   # ~0.5 sn öngörü (viraj yaklaşırken önceden yavaşla)
