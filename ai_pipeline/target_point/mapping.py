"""Faz 1 harita çıkarımı: simülatörü sürüp pist haritası üretir.

Veri toplama pipeline'ının İLK adımıdır. CleanMappingTeacher ile pisti
bir tur (veya açık yolda belirli mesafe) temiz sürer, her karenin gerçek
pose'unu kaydeder, sonra bunu build_track_map ile merkez çizgisine çevirip
diske yazar. Bu harita olmadan etiket üretilemez (target noktalar buna göre
hesaplanır). Karşılığı komut: collect_target_point_data.py --task map.
"""
...
def _trace_point_from_observation(observation, action, step_index: int, elapsed_sec: float, loop_index: int) -> MapTracePoint:
    """Simülatör gözlemini + teacher komutunu tek bir MapTracePoint'e paketler.
    Yani 'şu an' fotoğrafı: konum, yaw, hız, cte, direksiyon/gaz. Bu noktalar
    birikip ham izi (raw_trace) oluşturur."""
...
def build_phase1_map(
    ...
    """Bir pisti haritalar: temiz sür -> izi topla -> harita yap -> kaydet.

    Simülatörü açar, CleanMappingTeacher ile pisti sürer ve bir TUR tamamlamayı
    (başlangıca yaklaşma + yön hizası + yeterli ilerleme) ya da açık yolda
    hedef mesafeyi algılayınca durur. Çarpma/erken done olursa hata verir.
    Sonunda build_track_map + save_track_map ile <output_root>/<map_id>/
    altına metadata.json, raw_trace.csv, centerline.csv yazar.
    Döner: map_id, output_dir, toplam uzunluk ve nokta sayıları içeren sözlük."""
