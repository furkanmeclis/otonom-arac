"""Pist haritası veri yapıları ve geometri yardımcıları.

Bu modül, etiketlemenin üzerine kurulduğu 'pist haritasını' tanımlar ve
diske kaydeder/okur. Faz 1'de teacher pisti sürerken ham iz (raw_trace)
toplanır; bu modül onu temiz, eşit aralıklı bir MERKEZ ÇİZGİSİNE (centerline)
dönüştürür ve her noktaya yön, eğrilik, referans hız bilgisini ekler.

Ana veri yapıları:
  * MapTracePoint     : ham izin tek karesi (konum, yaw, hız, direksiyon...).
  * CenterlineWaypoint: işlenmiş merkez-çizgi noktası (konum + yön + eğrilik +
                        referans hız). Etiketler bunlardan üretilir.
  * TrackMapArtifact  : bir pistin tam haritası (ham iz + centerline + meta).

Ana fonksiyonlar: build_track_map (iz -> harita), save/load_track_map
(disk: metadata.json + raw_trace.csv + centerline.csv), ve
interpolate_centerline_position (çizgi boyunca metre cinsinden ara nokta).
Koordinatlar dünya ekseninde; yatay düzlem (x, z) kullanılır.
"""
...
def build_track_map(
    ...
    """Ham sürüş izini (trace) düzgün bir pist haritasına dönüştürür.

    Adımlar: izi turlara böl -> turları hizalayıp ortala -> spacing_m
    aralıkla yeniden örnekle -> her noktada yön, eğrilik ve referans hızı
    hesapla. closed=True kapalı pist (başa döner), False açık yol içindir.
    Döner: kaydedilmeye/etiketlemeye hazır TrackMapArtifact."""
...
def interpolate_centerline_position(
    ...
    """Merkez çizgisinde, başlangıçtan distance_m metre ileride dünya konumunu döndürür.

    Waypoint'ler arası doğrusal interpolasyon yapar. Kapalı pistte mesafe
    toplam uzunluğa göre mod alınır (sona gelince başa sarar), açık pistte
    [0, toplam] aralığına sıkıştırılır. Etiketlemede 'lookahead noktasını'
    bulmak için kullanılır. Döner: [x, y, z] dünya koordinatı."""
...
def save_track_map(artifact: TrackMapArtifact, output_root: str | Path, extra_metadata: Dict[str, object] | None = None) -> Path:
    """Pist haritasını diske yazar: <output_root>/<map_id>/ altına
    metadata.json + raw_trace.csv + centerline.csv. Bu üç dosya,
    build_target_point_labels.py'nin --map-dir ile beklediği formattır.
    Döner: oluşturulan klasörün yolu."""
...
def load_track_map(map_dir: str | Path) -> TrackMapArtifact:
    """Diskten pist haritasını okur (save_track_map'in tersi).
    map_dir içinde metadata.json + raw_trace.csv + centerline.csv bekler;
    bunları tekrar bir TrackMapArtifact nesnesine birleştirir. Etiketleme
    ve değerlendirme bu fonksiyonla haritayı yükler."""
