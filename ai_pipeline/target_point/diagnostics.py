"""Eğitim teşhisi ve görselleştirme yardımcıları.

Eğitilen modelin 'sağlığını' ölçer ve raporlar (açık-döngü). Tahminleri
gerçek etiketlerle karşılaştırır: hata, korelasyon, tahmin yayılımı,
senaryo/pist bazında dökümler ve görsel 'contact sheet' (tahmin vs etiket
örnek kareleri). En önemlisi evaluate_collapse_gate: modelin çökmediğini
(girdiye duyarlı olduğunu, özellikle virajları öğrendiğini) doğrulayan geçit.
training.py bu fonksiyonları eğitim sonunda çağırır.
"""
...
def summarize_predictions(model, samples: Sequence[TargetPointSample], cfg, split_name: str) -> Dict[str, object]:
    """Modelin örnekler üzerindeki tahminlerini özetler (hata, korelasyon, yayılım).
    Tahmin ile etiketi karşılaştırıp metrik sözlüğü döndürür; düz/viraj gibi
    segmentlere de ayırır. Çökme kontrolü ve raporlama bu özetleri kullanır."""
...
def evaluate_collapse_gate(train_metrics: Dict[str, object], val_metrics: Dict[str, object]) -> Dict[str, object]:
    """Modelin 'çökmediğini' doğrulayan geçit (kalite kapısı).
    Tahmin yayılımı, korelasyon ve özellikle VİRAJ performansı gibi ölçütleri
    eşiklerle karşılaştırır. Geçemezse model işe yaramaz sayılır (girdiye
    duyarsız, hep ortalamayı tahmin ediyor olabilir). Döner: geçti/kaldı + sebepler."""
