"""Veri kümesi yükleme: tub'lardan ve JSONL manifest'lerden örnek üretimi.

Bu modül eğitime giren veriyi HAZIRLAR. İki kaynaktan örnek (TargetPointSample)
üretebilir ve gerektiğinde karıştırır:

  * JSONL manifest (sim): build_target_point_labels.py'nin ürettiği, her satırı
    bir kareye karşılık gelen (image_path + target_x/target_y + meta) dosyalar.
  * Ham tub (eski yol / gerçek veri): doğrudan DonkeyCar tub'larını okuyup,
    ardışık konumlardan lookahead ile target noktayı kendisi hesaplar.

Temel kavramlar:
  * TargetPointSample : tek eğitim örneği (görüntü yolu + etiket + senaryo bilgisi).
  * compute_target_point/world_to_ego : ham konumlardan ego-frame hedef üretimi.
  * split_samples_by_group : aynı tub'ın kareleri train/val'a BÖLÜNMESİN diye
    grup bazlı (sızıntısız) bölme.
  * load_target_point_splits / load_mixed_splits : eğitimin çağırdığı üst-seviye
    yükleyiciler; sim + gerçek veriyi config'teki orana göre harmanlar.
"""
...
class TargetPointSample:
    """Tek bir eğitim örneği: bir görüntü + onun hedef-nokta etiketi + meta.

    En önemli alanlar: image_path (girdi görüntüsü), target_x/target_y (etiket,
    ego-frame metre), group_id (sızıntısız bölme için; aynı grup ya train ya val).
    Diğer alanlar (senaryo, lookahead, teacher komutları) örnek ağırlıklandırma
    ve teşhis içindir."""
    image_path: str
    target_x: float
    target_y: float
...
def world_to_ego(current_position: np.ndarray, heading: np.ndarray, target_position: np.ndarray) -> Tuple[float, float]:
    """Dünya konumundaki hedefi, aracın ego-frame'ine çevirir.
    heading 'ileri' eksenidir; sağ dik vektör 'sağ' eksenidir. Hedefe olan
    fark bu eksenlere izdüşürülür. Döner: (target_x=sağ, target_y=ileri) metre."""
...
def compute_target_point(
    ...
    """Ham konum dizisinden tek kare için hedef noktayı hesaplar.
    'index' anındaki konumdan lookahead_meters kadar ileri giden noktayı
    (gelecekteki konumu) bulur, o anki yönü tahmin eder, ego-frame'e çevirir.
    Yeterli ileri veri/yön yoksa None döner. Tub tabanlı etiketlemenin özü."""
...
def split_samples_by_group(
    ...
    """Örnekleri GRUP bazında train/val'a böler (veri sızıntısını önler).

    Aynı tub/bölümün ardışık kareleri çok benzerdir; rastgele bölersek aynı
    sahnenin bir karesi train'e, komşusu val'e düşer ve val sahte yüksek çıkar.
    Bu yüzden grupların TAMAMI ya train'e ya val'e gider. seed ile deterministik.
    Döner: (train, val, grup sayıları)."""
...
def load_target_point_manifest_splits(
    ...
    """Hazır JSONL manifest'ten (sim) train/val örneklerini yükler.
    label_mode'a göre doğru manifest dosyalarını bulur ve satırları
    TargetPointSample'a çevirir. Sim verisinin ana yükleme yolu."""
...
def load_target_point_splits(
    ...
    """Birincil (sim) veriyi yükler: manifest varsa ondan, yoksa ham tub'lardan.
    Yani kaynağa göre otomatik seçim yapan akıllı yükleyici. Gerçek/harici
    veriyle karıştırma load_mixed_splits'te yapılır; bu fonksiyon tek kaynaktır."""
...
def load_mixed_splits(
    ...
    """Sim + harici (gerçek) veriyi config'teki oranda harmanlayan üst yükleyici.

    Eğitimin gerçekte çağırdığı fonksiyon. cfg'ye göre:
      - EXTERNAL_ONLY=True ise sadece gerçek veri (model_03 gibi),
      - EXTERNAL_ROOT yoksa sadece sim,
      - ikisi de varsa EXTERNAL_DATA_RATIO oranında karıştırır (hibrit modeller).
    Harici veri scan_external_datasets ile pseudo-label'lı okunur. Bu, model_03/
    05/06 gibi config'lerin sim-gerçek dengesini belirlediği yerdir."""
