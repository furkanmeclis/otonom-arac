"""Kapalı-döngü (closed-loop) simülatör değerlendirmesi.

Modelin GERÇEKTEN sürüp süremediğini ölçer. 'Açık-döngü' metrikler (val
kaybı) yanıltıcı olabilir; burada model simülatörde aracı kendisi sürer
(pilot -> controller -> simülatör -> yeni görüntü -> pilot ...) ve sonuçlar
ölçülür: tur tamamlama oranı, pist dışına çıkma/çarpma sayısı, kat edilen
mesafe, direksiyon titremesi vb. evaluate_target_point.py bunu çağırır.

Ana fonksiyonlar: evaluate_episode (tek bölümü koşup metrik üretir) ve
evaluate_closed_loop (tüm pistler/bölümler üzerinde toplar, rapor yazar).
Bu, bir modelin 'iyi mi kötü mü' olduğuna karar verdiğimiz nihai testtir.
"""
...
def evaluate_episode(
    ...
    """Tek bir bölümü modelle koşar ve performans metriklerini üretir.
    Pilot + controller ile aracı sürer; her karede modelin tahminini komuta
    çevirir, simülatörü ilerletir. Tur tamamlama, pist-dışı/çarpma, mesafe ve
    direksiyon düzgünlüğü gibi metrikleri toplayıp döndürür."""
...
def evaluate_closed_loop(
    ...
    """Bir modeli birçok pist ve bölümde değerlendiren üst fonksiyon.
    Her pist için evaluate_episode'u episodes_per_track kez koşar, sonuçları
    pist ve genel düzeyde toplar, JSON rapor yazar. evaluate_target_point.py'nin
    çağırdığı giriş noktası. Döner: tüm metrikleri içeren özet sözlük."""
