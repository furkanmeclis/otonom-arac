"""Keras hedef-nokta modelini Jetson için TFLite'a (INT8/FP16/FP32) dönüştürür.

Eğitimden çıkan .keras modeli, gömülü donanımda hızlı çalışacak .tflite
formatına çevirir. INT8 niceleme en küçük/en hızlı sonucu verir ama doğruluğu
korumak için 'kalibrasyon' görüntüleri gerekir (representative dataset);
FP16 daha az hızlı ama kalibrasyonsuz, FP32 ise niceleme yok demektir.
benchmark_tflite ile üretilen modelin gecikmesi (ms) ölçülebilir. Çıktı
pilot_tflite.py tarafından gerçek araçta kullanılır.
"""
...
def _representative_dataset_generator(
    ...
    """INT8 kalibrasyonu için 'temsili veri' üreten fonksiyon döndürür.
    TFLite, INT8 niceleme ölçeklerini belirlemek için gerçek görüntü örnekleri
    görmek ister; bu jeneratör image_dir'den num_samples kareyi ön-işleyip verir."""
...
def export_tflite(
    ...
    *,
    ...
    """Bir .keras modelini .tflite formatına dönüştürür (ana dışa aktarım fonksiyonu).

    quantize='int8' ise calibration_image_dir gerekir; 'float16'/'none' kalibrasyonsuz.
    Döner: yazılan .tflite dosyasının yolu.
...
def benchmark_tflite(
    ...
    """TFLite modelinin çıkarım hızını ölçer (gecikme testi).

    Modeli num_runs kez koşturup süre istatistiği döndürür; Jetson'da
    gerçek-zamanlı (yeterince hızlı) olup olmadığını anlamak için kullanılır.
