"""DonkeySim simülatörü ile deterministik oturum (session) yardımcıları.

Simülatörle konuşan ince bir sarmalayıcı (wrapper). gym_donkeycar ortamını
açar/kapatır ve her adımda (steering, throttle, brake) komutunu gönderip
karşılığında zengin bir gözlem (SimObservation) döndürür: görüntü + GERÇEK
pose (konum, yaw), cte (merkeze uzaklık), hız, çarpma bilgisi, tur sayısı...

Bu 'ayrıcalıklı' bilgiyi (gerçek pose'u) yalnızca etiketleme ve veri toplama
kullanır; model bunları görmez, sadece görüntüyü görür. Harita çıkarma, veri
toplama ve kapalı-döngü değerlendirmenin hepsi bu oturum üstünde çalışır.
'with DonkeySimSession(...) as s:' kalıbıyla güvenli açılıp kapanır.
"""
...
class SimObservation:
    """Simülatörün tek adımda döndürdüğü gözlem paketi.
    image = modele girecek kamera görüntüsü; pos/yaw_deg/cte/speed/hit = sadece
    etiketleme ve değerlendirmede kullanılan 'ayrıcalıklı' gerçek durum bilgisi."""
    image: np.ndarray
    reward: float
    done: bool
...
class DonkeySimSession:
    """Donkey simülatör gym ortamı etrafında ince, deterministik sarmalayıcı.
    open/close ile ortamı yönetir, step() ile komut gönderip gözlem alır,
    reset()/prime() ile başa sarar. seed sabitlenir, böylece koşular tekrarlanabilir."""
    ...
    def step(self, steering: float, throttle: float, brake: float = 0.0) -> SimObservation:
        """Simülatöre tek komut gönderir, bir adım ilerletir, gözlem döndürür.
        brake > 0 ise gaz sıfırlanır. Telemetriyi (pose, yaw, cte, hız, çarpma)
        ayrıştırıp SimObservation olarak verir. Adım sayacını ve geçen süreyi günceller."""
        if self.env is None:
            self.open()
    ...
    def prime(self) -> SimObservation:
        """Ortamı sıfırlar ve ilk (telemetri içeren) gözlemi alır.
        reset sonrası ilk step boş telemetri dönebildiği için sıfır komutla bir
        adım atıp dolu bir SimObservation garantiler. Bölüm başında çağrılır."""
        self.reset()
