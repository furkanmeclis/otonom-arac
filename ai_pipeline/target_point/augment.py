"""Eğitim sırasında uygulanan deterministik veri artırma (augmentation).

Veri artırma, aynı görüntünün hafifçe değiştirilmiş kopyalarını üreterek
modelin ezberlemesini önler ve genellemesini artırır (parlaklık, kontrast,
hafif kayma/dönme, RGB kaydırma, bulanıklık...). Buradaki artırma 'klip
tutarlı' ve deterministiktir: aynı kısa zaman dilimine (ardışık kareler) AYNI
rastgele dönüşüm uygulanır ve seed sabitse hep aynı sonucu verir. Böylece
ardışık karelerin etiketleri (hedef nokta) tutarlı kalır. Yalnızca eğitimde
çalışır; gerçek sürüşte (çıkarım) uygulanmaz.
"""
...
class TemporalAugmentationParams:
    """Tek bir klibe uygulanacak artırma parametrelerinin demeti.
    (parlaklık, kontrast, bulanıklık, RGB kaydırma, yatay kayma, dönme,
    perspektif). Bir klip için bir kez örneklenir, tüm karelerine uygulanır."""
    brightness: float
    contrast: float
    blur_radius: float
    rgb_shift_r: int
...
class TemporalAugmenter:
    """Kısa bir zaman klibi içinde paylaşılan deterministik artırmayı uygular.
    enabled=False ise hiçbir şey yapmaz (geçişsiz). Aynı (seed, klip) için her
    zaman aynı dönüşümü üretir; bu sayede eğitim tekrarlanabilir ve ardışık
    karelerin etiketleri bozulmaz."""
