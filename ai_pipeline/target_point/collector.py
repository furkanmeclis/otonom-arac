"""Faz 2/3 veri toplama: harita üstünde gürültülü sürüşle örnek üretir.

Faz 1'de (mapping.py) çıkarılan pist haritasını kullanarak simülatörde
çok sayıda sürüş bölümü (episode) koşar ve her kareyi (görüntü + teacher'ın
ürettiği TEMİZ etiket + uygulanan komut/senaryo) ham veri olarak kaydeder.

İçindeki ana toplama fonksiyonları:
  * collect_dataset()        : bir aşamanın TÜM pistlerini/bölümlerini toplar (üst düzey).
  * collect_episode()        : tek bir bölümü koşar (LowNoiseTargetPointTeacher ile).
  * collect_rollout_dataset()/collect_model_rollout_episode():
        EĞİTİLMİŞ modeli sürdürerek veri toplar (DAgger benzeri); modelin
        kendi hatalarına teacher düzeltmesi ekleyip dağılım kaymasını azaltır.
  * collect_generated_roads_episode_from_mapping(): açık/üretilen yollar için özel.
  * RawEpisodeSample          : kaydedilen tek ham karenin veri yapısı.

collection_profile (phase2_low_noise / phase3_full_noise) gürültü/kurtarma
şiddetini belirler. Çıktı: <dataset_root>/raw altında bölüm klasörleri +
index; sonra build_target_point_labels.py bunları manifest'e çevirir.
"""
...
class RawEpisodeSample:
    """Toplama sırasında kaydedilen tek ham kare (henüz manifest'e dönüşmemiş).
    Görüntü yolu, temiz etiket (target_x/y), uygulanan komut, senaryo ve pose
    bilgisini taşır. build_target_point_labels.py bunları işleyip eğitime hazırlar."""
    sample_id: str
    episode_id: str
    frame_index: int
    elapsed_sec: float
...
def collect_episode(
    ...
    """Tek bir bölümü (episode) koşar ve karelerini kaydeder.
    Pistin haritasını yükler, LowNoiseTargetPointTeacher ile sürer; her karede
    görüntüyü ve teacher'ın temiz etiketini + uygulanan komutu RawEpisodeSample
    olarak diske yazar. seed ile tekrarlanabilir. Döner: bölüm özeti (kare sayısı vb.)."""
    dataset_root = Path(dataset_root).resolve()
...
def collect_dataset(
    ...
    """Bir aşamanın tüm pistleri ve bölümleri için veri toplar (üst düzey).
    split_tracks (train/val pist listeleri) üzerinden gider, her pist için
    episodes_per_track bölüm koşar (collect_episode), ham örnekleri ve indeksi
    yazar. collection_profile gürültü seviyesini seçer. Döner: toplama özeti."""
    dataset_root = Path(dataset_root).resolve()
...
def collect_rollout_dataset(
    ...
    """EĞİTİLMİŞ modeli sürdürerek veri toplar (rollout / DAgger benzeri).
    Model kendi sürer, araç hata yapıp dağılımdan saptıkça teacher'ın TEMİZ
    etiketi 'doğru cevap' olarak kaydedilir. Böylece model, kendi hatalarından
    dönmeyi öğrenir ve compounding-error azalır. driver_model_path verilmezse
    en iyi model otomatik seçilir. Döner: toplama özeti."""
    dataset_root = Path(dataset_root).resolve()
    index_root = dataset_root / "index"
    index_root.mkdir(parents=True, exist_ok=True)
