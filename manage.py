...
Usage:
    manage.py (drive) [--model=<model>] [--js] [--type=(linear|categorical|target_point)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--simulationconfig=<filename>]
    manage.py (train) [--tubs=tubs] (--model=<model>) [--type=(linear|inferred|tensorrt_linear|tflite_linear|target_point)]
    manage.py (smoke) [--simulationconfig=<filename>]
...
Options:
    ...
    --simulationconfig=filename  Specify simulation config file to use.
                            [default: simulationconfig.py]
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ai_pipeline'))
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #
    if model_path:
        use_target_point_pilot = model_type == "target_point"
        if use_target_point_pilot:
            from target_point.controller import TargetPointController
            from target_point.pilot import TargetPointPilot
            if not (model_path.endswith(".h5") or model_path.endswith(".keras") or model_path.endswith(".tflite")):
                print("ERR>> target_point supports .h5, .keras, or .tflite model files.")
                return

        # If we have a model, create an appropriate Keras part
        if use_target_point_pilot and model_path.endswith(".tflite"):
            from target_point.pilot_tflite import TargetPointPilotTFLite
            kl = TargetPointPilotTFLite(cfg)
        elif use_target_point_pilot:
            kl = TargetPointPilot(cfg)
        else:
        ...
        if '.h5' in model_path or '.keras' in model_path or '.trt' in model_path or '.tflite' in \
        ...
        #
        outputs = ['pilot/target_x', 'pilot/target_y'] if use_target_point_pilot else ['pilot/angle', 'pilot/throttle']

        if cfg.TRAIN_LOCALIZER and not use_target_point_pilot:
        ...
        #
        if (not use_target_point_pilot) and (hasattr(cfg, 'TRANSFORMATIONS') or hasattr(cfg, 'POST_TRANSFORMATIONS')):
        ...
        if use_target_point_pilot:
            V.add(
                TargetPointController(cfg),
                inputs=['pilot/target_x', 'pilot/target_y'],
...
def smoke(cfg):
    """
    Launch the configured simulator once and verify the expected image and
    telemetry contract without starting any training or collection logic.
    """
    if not cfg.DONKEY_GYM:
        raise RuntimeError("Smoke test requires DONKEY_GYM=True.")
    ...
    conf = copy.deepcopy(dict(getattr(cfg, "GYM_CONF", {})))
    conf["exe_path"] = getattr(cfg, "DONKEY_SIM_PATH", "remote")
    conf["host"] = getattr(cfg, "SIM_HOST", "127.0.0.1")
    conf["port"] = int(conf.get("port", getattr(cfg, "DONKEY_SIM_PORT", 9091)))
    conf["guid"] = int(conf.get("guid", 0))
    conf["frame_skip"] = int(conf.get("frame_skip", 1))
    conf["start_delay"] = float(conf.get("start_delay", getattr(cfg, "DONKEY_SIM_START_DELAY", 5.0)))
    conf["cam_resolution"] = tuple(
        conf.get("cam_resolution", (int(cfg.IMAGE_H), int(cfg.IMAGE_W), int(cfg.IMAGE_DEPTH)))
    )
    conf.setdefault(
        "cam_config",
        {
            "img_w": int(cfg.IMAGE_W),
            "img_h": int(cfg.IMAGE_H),
            "img_d": int(cfg.IMAGE_DEPTH),
            "img_enc": "JPG",
            "fov": 60,
        },
    )

    expected_shape = (int(cfg.IMAGE_H), int(cfg.IMAGE_W), int(cfg.IMAGE_DEPTH))
    steps = max(1, int(getattr(cfg, "SMOKE_TEST_STEPS", 5)))
    steering = float(getattr(cfg, "SMOKE_TEST_STEERING", 0.0))
    throttle = float(getattr(cfg, "SMOKE_TEST_THROTTLE", 0.2))

    print(f"[smoke] env={cfg.DONKEY_GYM_ENV_NAME} sim={cfg.DONKEY_SIM_PATH}")
    print(
        f"[smoke] expected_shape={expected_shape} start_delay={conf['start_delay']} "
        f"host={conf['host']} port={conf['port']}"
    )

    env = gym.make(cfg.DONKEY_GYM_ENV_NAME, conf=conf)
    try:
        obs = env.reset()
        actual_shape = tuple(int(v) for v in getattr(obs, "shape", ()))
        if actual_shape != expected_shape:
            raise RuntimeError(f"Smoke test frame shape mismatch: expected {expected_shape}, got {actual_shape}")
        ...
        for step in range(steps):
            ...
            required_keys = ("pos", "cte", "speed", "lap_count")
            missing_keys = [key for key in required_keys if key not in info]
            if missing_keys:
                raise RuntimeError(f"Smoke test telemetry missing keys: {missing_keys}")

            print(
                "[smoke] step=%d reward=%.4f done=%s pos=%s cte=%.4f speed=%.4f lap_count=%s"
                % (
                    step,
                    float(reward),
                    bool(done),
                    info.get("pos"),
                    float(info.get("cte", 0.0)),
            ...
            if done:
                print("[smoke] simulator reported done early; stopping after validating telemetry.")
                break
...
def add_user_controller(V, cfg, use_joystick, input_image='ui/image_array'):
    ...
    :param cfg: the configuration (from simulationconfig.py)
